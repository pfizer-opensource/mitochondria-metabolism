...
%% Beta Oxidation Variable assignments
	...
	global_par_Vfcact=boxVmax(1);           % 0.42 %uM/min; -> M/s 
	global_par_Vrcact=boxVmax(2);          
	global_par_KmcactCarMAT=130.0/SF;       %M
	global_par_KmcactCarCYT=130.0/SF;
	global_par_KicactCarCYT= 200/SF;        %param(45) in IsoMIto;
	global_par_Keqcact=1.0;
	global_par_Vcpt2=boxVmax(3);             % 0.391 uM/min -> 6.5167E-9 M/s
	...
	global_par_Vvlcad=boxVmax(4);            % 0.0080 umol/min/mg protein; 
...
%% Flux Reactions for Beta-oxidation
	reaction_vcpt1C16_Keqcpt1=0.45;
    reaction_vcpt1C16_Kicpt1MalCoACYT=9.1/SF;
    reaction_vcpt1C16_Kmcpt1C16AcylCarCYT=Kmcpt1C16AcylCarCYT;  %136.0/SF;
    ...
    reaction_vcactC16_KicactC16AcylCarCYT=56.0/SF;  %param 44 in IsoMito
    ...
    reaction_vcactC14_KicactC14AcylCarCYT= 56.0/SF; %param(44); in Iso MIto
    ...
    reaction_vcactC12_KicactC12AcylCarCYT= 56.0/SF; %param(44); In Iso MIto
    ...
    reaction_vcactC10_KicactC10AcylCarCYT= 56.0/SF; %param(44) IsoMito
    ...
    reaction_vcactC8_KicactC8AcylCarCYT= 56.0/SF;   %param(44) IsoMito
    ...
    reaction_vcactC6_KicactC6AcylCarCYT=56.0/SF;    %param(44); %
    ...
    reaction_vcactC4_KicactC4AcylCarCYT=56.0/SF;    %param(44); %
    ...
    J_boxaccoa = ((1/compartment_VMAT)* ((reaction_vmckatC16)+(reaction_vmckatC14) + (reaction_vmckatC12) + ( reaction_vmckatC10) + (reaction_vmckatC8) + (reaction_vmckatC6) + ( 2.0 * reaction_vmckatC4) + (  reaction_vmtpC16) + ( reaction_vmtpC14) + (reaction_vmtpC12) + (reaction_vmtpC10) + (reaction_vmtpC8)))/W_x ;   %Acetyl-CoA production from beta oxidation
    J_boxnadh = ((1/compartment_VMAT)* (( 1.0 * reaction_vmschadC16)+ ( 1.0 * reaction_vmschadC14) + ( 1.0 * reaction_vmschadC12) + ( 1.0 * reaction_vmschadC10) + ( 1.0 * reaction_vmschadC8) + ( 1.0 * reaction_vmschadC6) + ( 1.0 * reaction_vmschadC4) + ( 1.0 * reaction_vmtpC16) + ( 1.0 * reaction_vmtpC14) + ( 1.0 * reaction_vmtpC12) + ( 1.0 * reaction_vmtpC10) + ( 1.0 * reaction_vmtpC8)))/W_x; %NADH production from beta oxidation
...
%  (iii) Buffer species
%f(iPYR_c) = (-Rm_cyto*J_PYRt)/W_c; % Commented out under the assumption pyruvate is always available
...
% fix [PYR] in the cytoplasm for in vivo experiments
f(iPYR_c) = 0;  %%Changed to test substrate availability


%% for control
if (iflag == 8) || (iflag == 11)
    % fix [PI] in the cytoplasm for in vivo experiments
    f(iPI_c) = 0;
elseif (iflag == 9) || (iflag == 12)
    f(iADP_c) = 0;
end
if (iflag == 101)
    f(iPCr_c) = 0;
