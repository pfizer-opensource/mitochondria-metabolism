function [Flux] = Cell_Flux(x,param,x_AtC,Varstruc)

boxVmax=Varstruc.boxVmax;
boxVmax(10)=Varstruc.Trimetazidine;

% This iteration of Test PDH is to include changes made to \beta-oxidation model to prevent conversion of acetyl-coA to ketoacyl-coA
...
Vmito = 0.2882*Varstruc.mito_rho;    % (ml mito / ml cell)
Vcyto = 0.94-Vmito;             % (ml cyto / ml cell)
...
compartment_VMAT = 1.8E-6;      % L mito/mg protein Bakker et al. (close to rho_m)


%  (iii) Pooled concentrations
Ctot   = 2.70e-3;             % M; total cytoC
Qtot   = 1.35e-3;             % M; total Q + QH2
SF=10^6;                      % Converts value from uM to M
NADtot = param(44);           % total NAD+NADH Bakker et al.
...
const_species_CoAMATt=param(45); %5000.0/SF; %M
CoAMAT=const_species_CoAMATt-(C16CoA_m+C16EnoylCoA_m+C16OHCoA_m+C16KetoCoA_m+ ...
    C14CoA_m+C14EnoylCoA_m+C14OHCoA_m+C14KetoCoA_m+C12CoA_m+C12EnoylCoA_m+ ...
    C12OHCoA_m+C12KetoCoA_m+C10CoA_m+C10EnoylCoA_m+C10OHCoA_m+C10KetoCoA_m+ ...
    C8CoA_m+C8EnoylCoA_m+C8OHCoA_m+C8KetoCoA_m+C6CoA_m+C6EnoylCoA_m+C6OHCoA_m+ ...
    C6KetoCoA_m+C4CoA_m+C4EnoylCoA_m+C4OHCoA_m+C4KetoCoA_m+ACCOA_x+SCOA_x);
...
kf=Kapp_CK*P_c(iATP)*P_c(iCr);
%J_CKf  = x_CK * (Kapp_CK*PCr_c); %
J_CKf=(kf*PCr_c*ADP_c); 
...
J_AtC  = (1/Rc_cell)*x_AtC; % DAB: changed to make constant 7/1/2020
...
% FADH2 and Coenzyme A are treated as fixed pools, (*note FADH2 does not
% play a role in complex II)
% NADH, and Acetyl-CoA variables are fixed to matrix NADH_x and ACCOA_x from
% TCA/OxPhos model 
% Beta-oxidation parameters are M/sec 
...
%% Enzyme functions for beta-oxidation
	...
	const_species_CarCYT=400.0/SF; %M
	const_species_CoACYT=140.0/SF; %M  %% fixed to zero in IsoMito
	const_species_MalCoACYT=param(43);

	const_species_CarMAT=950.0/SF; %M

	const_species_FADtMAT=0.77/SF; %M
	const_species_NADtMAT=NADtot;  %M
...
%% Beta Oxidation Variable assignments

	global_par_Vfcact=boxVmax(1);               % 0.42 uM/min
	global_par_Vrcact=boxVmax(2);               % 0.42 uM/min
	global_par_KmcactCarMAT=130.0/SF;         
	global_par_KmcactCarCYT=130.0/SF;
	global_par_KicactCarCYT= 200/SF;    
	global_par_Keqcact=1.0;
	global_par_Vcpt2=boxVmax(3);                % 0.391 uM/min
	...
	global_par_Vvlcad=boxVmax(4);               % 0.0080 uM/min
	global_par_KmvlcadC16AcylCoAMAT=6.5/SF;    
	global_par_KmvlcadC14AcylCoAMAT=4.0/SF;   
	global_par_KmvlcadC12AcylCoAMAT=2.7/SF;    
	...
	global_par_Vlcad=boxVmax(5);                % 0.01 uM/min
	...
	global_par_Vmcad=boxVmax(6);                % 0.081 uM/min
	...
	global_par_Vscad=boxVmax(7);               % 0.081 uM/min     
	...
	global_par_Vcrot=boxVmax(8);               % 3.6 uM/min     
	...
	global_par_Vmschad=boxVmax(9);                  % 1.0 uM/min
	...
	global_par_Vmckat=boxVmax(10);                  % 0.377 uM/min            
	...
	global_par_Vmtp=boxVmax(11);                    % 2.84 uM/min
...
%%  Variable C16AcylCoACYT option to fix time-course of C16AcylCoACYT (used for IsoMito Experiments)
	% C16AcylCoACYT=(26.8/SF)*2.71828182845905.^((-0.18/SF3).*t);
    
%% Reactions  used to caclulate flux (unused except for reaction_vcpt1C16 ; reaction_vcactC16 ; reaction_vcactC14 ;)
    ...
    reaction_vcpt1C16_Vcpt1=boxVmax(12) ;               %0.012 uM/min
    ...
    reaction_vcactC16_KicactC16AcylCarCYT=56.0/SF;  
    ...
    reaction_vcactC14_KicactC14AcylCarCYT= 56.0/SF; 
    ...
    reaction_vcactC12_KicactC12AcylCarCYT= 56.0/SF; 
    ...
    reaction_vcactC10_KicactC10AcylCarCYT= 56.0/SF; 
    ...
    reaction_vcactC8_KicactC8AcylCarCYT= 56.0/SF; 
    ...
    reaction_vcactC6_KicactC6AcylCarCYT=56.0/SF; 
    ...
    reaction_vcactC4_KicactC4AcylCarCYT=56.0/SF; 
    ...
    reaction_vcpt2C8_sfcpt2C8=0.35; %%%%%%%% delta sf1
    ...
    reaction_vcpt2C6_sfcpt2C6=0.15; %%%%%% delta sf2
    ...
    reaction_vlcadC16_sflcadC16= 0.9; %%% delta_sf(3)
    ...
    reaction_vmcadC8_sfmcadC8=0.87; %%%delta_sf(4)
    ...
    reaction_vmcadC6_sfmcadC6=1.0;  %%%delta SF5
...
%%  Defining indices for all fluxes
    ...
    Flux(i_J_vcpt1C16) = reaction_vcpt1C16 ;
    Flux(i_J_vcactC16) = reaction_vcactC16 ;
    Flux(i_J_vcactC14) = reaction_vcactC14 ;
    Flux(54) = J_CKf;
