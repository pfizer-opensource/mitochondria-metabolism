...
shade(2).struc(:)=[0.5843 0.8157 0.9882]; 
...
% Plotting Mean and 90% Prediction Interval
  BWoutmean=mean(BW_tpl([2:end],:));
  BWoutstd=std(BW_tpl([2:end],:));
  N = numel(BW_tpl([2:end],1)); 
  BWoutSEM=BWoutstd/sqrt(N); % Can be used to calculate CI of the mean
  CI95 = tinv([0.05 0.95], N-1); 
  yCI95 = bsxfun(@times, BWoutstd, CI95(:));   % Calculate Prediction interval
  ...
  figure(fig_num);
  hold on
  plot(NP,BWoutmean,colors(color_flag),'linewidth',2) 
