function [Flux, Fluxbox] = Mito_Flux(x,param,boxVmax,delta_sf)
...
%   rat=acetylcoA_x/COAS_x*NADH_x/NAD_x;
%   rat = (1e-5)/pyruvate_x;
% rat = 1;
...
Kapp_PYR_H = 1;
DGappo_PYR_H = -RT*log(Kapp_PYR_H);
...
Kapp_GLU_H = 1;
DGappo_GLU_H = -RT*log(Kapp_GLU_H);
...
Kapp_CIT_MAL = 1;
DGappo_CIT_MAL = -RT*log(Kapp_CIT_MAL);
...
Kapp_AKG_MAL = 1;
DGappo_AKG_MAL = -RT*log(Kapp_AKG_MAL);
...
Kapp_MAL_PI = 1;
DGappo_MAL_PI = -RT*log(Kapp_MAL_PI);
...
Kapp_SUC_MAL = 1;
DGappo_SUC_MAL = -RT*log(Kapp_SUC_MAL);
...
Keq_ANT = exp(-F*dPsi/RT);
Kapp_ANT = Keq_ANT *(P_i(iATP)*P_x(iADP)/P_x(iATP)/P_i(iADP));
DGappo_ANT = -RT*log(Kapp_ANT);
...
J_ANT = x_ANT/7.2679e-003*(0.70e-1)*num/den;
...
Kapp_Hle = exp(F*dPsi/RT);
DGappo_Hle = -RT*log(Kapp_Hle);

%% -------------------------------
% 8. K+/H+ anti-porter
J_KH   = x_KH*( K_i*H_x - K_x*H_i);
Kapp_KH = 1;
DGappo_KH = -RT*log(Kapp_KH);
...
% % K_CK = exp(50.78/RT); 
% K_CK = 7.408e8; 
% Kapp_CKc = K_CK*H_c*P_c(iATP)*P_c(iCr)/P_c(iADP)/P_c(iPCr);
% PCr_c = x(iPCr_c);
% Cr_c   = x(iCr_c);
% J_CKe  = x_CK * (Kapp_CKc*ADP_c*PCr_c - ATP_c*Cr_c );
% DGappo_CKc = -RT*log(Kapp_CKc);
% 
% 
% %% ------------------------------------------
...
% Keq_AK = 3.97e-1;
% Kapp_AKi = Keq_AK*P_i(iATP)*P_i(iAMP)/(P_i(iADP)*P_i(iADP));
% Kapp_AKc = Keq_AK*P_c(iATP)*P_c(iAMP)/(P_c(iADP)*P_c(iADP));
% x_AK = 1e7;
% J_AKi  = 0*x_AK*( Kapp_AKi*ADP_i*ADP_i - AMP_i*ATP_i );
% J_AKe  = x_AK*( Kapp_AKc*ADP_c*ADP_c - AMP_c*ATP_c );
% DGappo_AKc = -RT*log(Kapp_AKc);
% 
% 
% %% -------------------------------------------
% % 12. ATPase flux
...
% 
% DGappo_AtC = -RT*log(Kapp_AtC);

%% ATP hydrolysis potential in the matrix
% ATP4- + H2O = ADP3- + PI2- + H+
% x_AtC [=] mmol s^{-1} (l cell}^{-1}
% J_AtC [=] mmol s^{-1} (l cyto}^{-1}
% dGr_AtCo = dGf1(iADP) + dGf1(iPI) - dGf1(iATP) - dGf1(iH2O) ;
% Keq_AtC = exp(-dGr_AtCo/RT);
Keq_AtX = 1.16e-1;
Kapp_AtX = Keq_AtX*1/H_x*P_x(iADP)*P_x(iPI)/P_x(iATP);
dGr_AtXapp = -RT*log(Kapp_AtX);
DGappo_AtX = dGr_AtXapp + RT*log(PI_x*ADP_x/ATP_x);
...
% NADH_x, J(iCoAMATO) = 0  
% SS. Comment out AcetylC6CoACYT decline and fix flux to 0
% Step 1. Add in flux 
...
SF3 =60;   % s/min
	...
	const_species_CoACYT=0; %140.0/SF; %M Pool of CoA in the cytosol for cpt1
...
%% Beta oxidation Paramters
    ...
    const_species_CoAMATt=5000.0/SF;    % M
	compartment_VCYT = 10^-2; 			% L/mg X 1 mg protein RXN Buffer volume ;%2.2e-6; %L cyto/mg protein       %0.01; %L/mg Conversion factor to go from mol/sec/mg to mol/sec/L .      Experimental Conditions: 0.5 mg mito protein / mL Buffer in 2 mL buffer total
	compartment_VMAT = 1.8E-6; 			% L mito/mg protein       % L/mg Conversion factor to go from mol/sec/mg to mol/sec/L
    VMAT = 1;
	
    
    
    
    global_par_Vfcact=boxVmax(1);       % M/s
	global_par_Vrcact=boxVmax(1);       %
	...
	global_par_Vlcad=boxVmax(5);     %0.01/VMAT/SF3/SF; %/SF/SF3;       V
	...
	global_par_Vmcad=boxVmax(6)   ;%0.081/VMAT/SF/SF3;     %V
	...
	global_par_Vscad=boxVmax(7);    %0.081/VMAT/SF/SF3;      %V
	...
	global_par_Vcrot=boxVmax(8); %3.6/VMAT/SF/SF3;     %V
	...
	global_par_Vmschad=boxVmax(9);    % 1.0/VMAT/SF/SF3;     %V
	...
	global_par_Vmckat=boxVmax(10);%0.377/VMAT/SF/SF3;            %V
	...
	global_par_Keqmckat=1051; % 1051.0 ; % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Keq MCKAT changed 11/13/19 LFM
	global_par_Vmtp=boxVmax(11);   %2.84/VMAT/SF/SF3;        %V
...
%% Reactions
    ...
    reaction_vcpt1C16_Vcpt1=boxVmax(12) ; %0.012/SF/SF3;
    ...
    reaction_vcpt2C16_sfcpt2C16=s_f(1); %0.85; %%%%%% s_f(1)
    ...
    reaction_vcpt2C14_sfcpt2C14=s_f(2);%1.0; %%%%%%%%
    ...
    reaction_vcpt2C12_sfcpt2C12=s_f(3); %0.95; %%%%%%%%
    ...
    reaction_vcpt2C10_sfcpt2C10=s_f(4); %0.95; %%%%%%%%
    ...
    reaction_vcpt2C8_sfcpt2C8=delta_sf(1); %s_f(5); %0.35; %%%%%%%%
    ...
    reaction_vcpt2C6_sfcpt2C6=delta_sf(2); %s_f(6); %0.15; %%%%%%
    ...
    reaction_vcpt2C4_sfcpt2C4=s_f(7); %0.01; %%%%%
    ...
    reaction_vvlcadC16_sfvlcadC16=s_f(8); %1.0; %%%%%
    ...
    reaction_vvlcadC14_sfvlcadC14=s_f(9); %0.42; %%%%%
    ...
    reaction_vvlcadC12_sfvlcadC12=s_f(10); %0.11;
    ...
    reaction_vlcadC16_sflcadC16=delta_sf(3); %s_f(11); % 0.9;
    ...
    reaction_vlcadC14_sflcadC14=s_f(12); %1.0;
    ...
    reaction_vlcadC12_sflcadC12=s_f(13); %0.9;
    ...
    reaction_vlcadC10_sflcadC10=s_f(14); %0.75;
    ...
    reaction_vlcadC8_sflcadC8=s_f(15); %0.4;
    ...
    reaction_vmcadC12_sfmcadC12=s_f(16); %0.38;
    ...
    reaction_vmcadC10_sfmcadC10=s_f(17);  %0.8;
    ...
    reaction_vmcadC8_sfmcadC8=delta_sf(4); %s_f(18);  %0.87;
    ...
    reaction_vmcadC6_sfmcadC6=delta_sf(5); %s_f(19); %1.0;
    ...
    reaction_vmcadC4_sfmcadC4=s_f(20); %0.12;
    ...
    reaction_vscadC6_sfscadC6=s_f(21); %0.3;
    ...
    reaction_vscadC4_sfscadC4=s_f(22); %1.0;
    ...
    reaction_vcrotC16_sfcrotC16=s_f(23); %0.13;
    ...
    reaction_vcrotC14_sfcrotC14=s_f(24); %0.2;
    ...
    reaction_vcrotC12_sfcrotC12=s_f(25); %0.25;
    ...
    reaction_vcrotC10_sfcrotC10=s_f(26); %0.33;
    ...
    reaction_vcrotC8_sfcrotC8=s_f(27); %0.58;
    ...
    reaction_vcrotC6_sfcrotC6=s_f(28); %0.83;
    ...
    reaction_vcrotC4_sfcrotC4=s_f(29); %1.0;
    ...
    reaction_vmschadC16_sfmschadC16=s_f(30); %0.6;
    ...
    reaction_vmschadC14_sfmschadC14=s_f(31); %0.5;
    ...
    reaction_vmschadC12_sfmschadC12=s_f(32); %0.43;
    ...
    reaction_vmschadC10_sfmschadC10=s_f(33); %0.64;
    ...
    reaction_vmschadC8_sfmschadC8=s_f(34); %0.89;
    ...
    reaction_vmschadC6_sfmschadC6=s_f(35); %1.0;
    ...
    reaction_vmschadC4_sfmschadC4=s_f(36); %0.67;
    ...
    reaction_vmckatC16_sfmckatC16=s_f(37); %0.0;
    ...
    reaction_vmckatC14_sfmckatC14=s_f(38); %0.2;
    ...
    reaction_vmckatC12_sfmckatC12=s_f(39);  %0.38;
    ...
    reaction_vmckatC10_sfmckatC10=s_f(40); %0.65;
    ...
    reaction_vmckatC8_sfmckatC8=s_f(41); %0.81;
    ...
    reaction_vmckatC6_sfmckatC6=s_f(42); %1.0;
    ...
    reaction_vmckatC4_sfmckatC4=s_f(43); %0.49;
    ...
    reaction_vmtpC16_sfmtpC16=s_f(44); %1.0;
    ...
    reaction_vmtpC14_sfmtpC14=s_f(45); %0.9;
    ...
    reaction_vmtpC12_sfmtpC12=s_f(46); %0.81;
    ...
    reaction_vmtpC10_sfmtpC10=s_f(47); %0.73;
    ...
    reaction_vmtpC8_sfmtpC8=s_f(48); %0.34;
    ...
    reaction_vnadhsink_K1nadhsink= 1.6584E-6; %16.0/SF; %LM change 9/18
...
J_boxaccoa=(1/(compartment_VMAT))*((reaction_vmckatC16)+(reaction_vmckatC14) + (reaction_vmckatC12) + ( reaction_vmckatC10) + (reaction_vmckatC8) + (reaction_vmckatC6) + ( 2.0 * reaction_vmckatC4) + (  reaction_vmtpC16) + ( reaction_vmtpC14) + (reaction_vmtpC12) + (reaction_vmtpC10) + (reaction_vmtpC8))/W_x ;   %((reaction_vmckatC16)+(reaction_vmckatC14) + (reaction_vmckatC12) + ( reaction_vmckatC10) + (reaction_vmckatC8) + (reaction_vmckatC6) + ( 2.0 * reaction_vmckatC4) + (  reaction_vmtpC16) + ( reaction_vmtpC14) + (reaction_vmtpC12) + (reaction_vmtpC10) + (reaction_vmtpC8));    %AcCoA production from beta oxidation
...
% J_CoAMAT = -((1/(compartment_VMAT))*(( 1.0 * reaction_vcpt2C16) + (-1.0 * reaction_vvlcadC16) + (-1.0 * reaction_vlcadC16))-(( 1.0 * reaction_vvlcadC16) + ( 1.0 * reaction_vlcadC16) + (-1.0 * reaction_vcrotC16) + (-1.0 * reaction_vmtpC16))-(( 1.0 * reaction_vcrotC16) + (-1.0 * reaction_vmschadC16))...
%     -(( 1.0 * reaction_vmschadC16) + (-1.0 * reaction_vmckatC16))-(( 1.0 * reaction_vcpt2C14) + (-1.0 * reaction_vvlcadC14) + (-1.0 * reaction_vlcadC14) + ( 1.0 * reaction_vmckatC16) + ( 1.0 * reaction_vmtpC16))...
%     -(( 1.0 * reaction_vvlcadC14) + ( 1.0 * reaction_vlcadC14) + (-1.0 * reaction_vcrotC14) + (-1.0 * reaction_vmtpC14))-(( 1.0 * reaction_vcrotC14) + (-1.0 * reaction_vmschadC14))-(( 1.0 * reaction_vmschadC14) + (-1.0 * reaction_vmckatC14))...
%     -(( 1.0 * reaction_vcpt2C12) + (-1.0 * reaction_vvlcadC12) + (-1.0 * reaction_vlcadC12) + (-1.0 * reaction_vmcadC12) + ( 1.0 * reaction_vmckatC14) + ( 1.0 * reaction_vmtpC14))...
%     -(( 1.0 * reaction_vvlcadC12) + ( 1.0 * reaction_vlcadC12) + ( 1.0 * reaction_vmcadC12) + (-1.0 * reaction_vcrotC12) + (-1.0 * reaction_vmtpC12))-(( 1.0 * reaction_vcrotC12) + (-1.0 * reaction_vmschadC12))...
%     -(( 1.0 * reaction_vmschadC12) + (-1.0 * reaction_vmckatC12))-(( 1.0 * reaction_vcpt2C10) + (-1.0 * reaction_vlcadC10) + (-1.0 * reaction_vmcadC10) + ( 1.0 * reaction_vmckatC12) + ( 1.0 * reaction_vmtpC12))...
%     -(( 1.0 * reaction_vlcadC10) + ( 1.0 * reaction_vmcadC10) + (-1.0 * reaction_vcrotC10) + (-1.0 * reaction_vmtpC10))-(( 1.0 * reaction_vcrotC10) + (-1.0 * reaction_vmschadC10))-(( 1.0 * reaction_vmschadC10) + (-1.0 * reaction_vmckatC10))...
%     -(( 1.0 * reaction_vcpt2C8) + (-1.0 * reaction_vlcadC8) + (-1.0 * reaction_vmcadC8) + ( 1.0 * reaction_vmckatC10) + ( 1.0 * reaction_vmtpC10))-(( 1.0 * reaction_vlcadC8) + ( 1.0 * reaction_vmcadC8) + (-1.0 * reaction_vcrotC8) + (-1.0 * reaction_vmtpC8))...
%     -(( 1.0 * reaction_vcrotC8) + (-1.0 * reaction_vmschadC8))-(( 1.0 * reaction_vmschadC8) + (-1.0 * reaction_vmckatC8))-(( 1.0 * reaction_vcpt2C6) + (-1.0 * reaction_vmcadC6) + (-1.0 * reaction_vscadC6) + ( 1.0 * reaction_vmckatC8) + ( 1.0 * reaction_vmtpC8))...
%     -(( 1.0 * reaction_vmcadC6) + ( 1.0 * reaction_vscadC6) + (-1.0 * reaction_vcrotC6))-(( 1.0 * reaction_vcrotC6) + (-1.0 * reaction_vmschadC6))-(( 1.0 * reaction_vmschadC6) + (-1.0 * reaction_vmckatC6))...
...
%%  Defining indices for all fluxes
    ...
    i_J_boxnadh = 46; 
    i_J_boxaccoa =47;
    i_J_reaction_vmschadC16=48;
    i_J_CoAMAT = 49; 
    i_J_boxcoash = 50;
    
%     i_J_xxx = 46; % This line should be commented during the optimization
	...
	Flux(i_J_KH)	  = J_KH; 
	Flux(i_J_pdh)     = J_pdh; 
	Flux(i_J_cits)    = J_cits; 
	Flux(i_J_acon)    = J_acon; 
	Flux(i_J_isod)    = J_isod; 
	Flux(i_J_akgd)    = J_akgd; 
	Flux(i_J_scoas)   = J_scoas;
	Flux(i_J_sdh)	  = J_sdh; 
	Flux(i_J_fum)     = J_fum; 
	Flux(i_J_mdh)	  = J_mdh; 
	Flux(i_J_ndk)	  = J_ndk; 
	...
	Flux(i_J_ADPt)    = J_ADP; 
	Flux(i_J_AMPt)    = J_AMP;
	Flux(i_J_PYR_H)   = J_PYR_H; 
	Flux(i_J_GLU_H)   = J_GLU_H; 
	Flux(i_J_CIT_MAL) = J_CIT_MAL; 
	Flux(i_J_SUC_MAL) = J_SUC_MAL; 
	Flux(i_J_AKG_MAL) = J_AKG_MAL; 
	Flux(i_J_MAL_PI)  = J_MAL_PI; 
	Flux(i_J_ASP_GLU) = J_ASP_GLU; 
	Flux(i_J_PYRt)    = J_PYRt; 
	Flux(i_J_CITt)    = J_CITt; 
	Flux(i_J_ICITt)   = J_ICITt; 
	Flux(i_J_MALt)    = J_MALt; 
	Flux(i_J_AKGt)    = J_AKGt; 
	Flux(i_J_SUCt)    = J_SUCt; 
	Flux(i_J_FUMt)    = J_FUMt; 
	Flux(i_J_GLUt)    = J_GLUt; 
	Flux(i_J_ASPt)    = J_ASPt; 
	Flux(i_J_CKe)	  = 0;
	Flux(i_J_AKe)	  = 0;
	Flux(i_J_AtC)     = 0;
    ...
    Fluxbox(3) = reaction_vcactC6;
    Fluxbox(4) = reaction_vcactC8;
    Fluxbox(5) = reaction_vcactC10;
    Fluxbox(6) = reaction_vcactC12;
    Fluxbox(7) = reaction_vcactC14;
    Fluxbox(8) = reaction_vcactC16;
         
         Fluxbox(9)=reaction_vmtpC16 ; %  
         Fluxbox(10) = reaction_vmckatC14; 
         Fluxbox(11)= reaction_vmckatC12 ;
         ...
         Fluxbox(19) = reaction_vmtpC8;  %   
         Fluxbox(20) = reaction_vmckatC16; 
         
         Fluxbox(21) = reaction_vcrotC16; 
         Fluxbox(22) = reaction_vmschadC16;
         Fluxbox(23) = COASH_x;
         
         
    
%     Flux(i_J_xxx)     = dGr_AtC;  % This line should be commented during the optimization
